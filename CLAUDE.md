...
**Тестов нет** — не придумывать команды `npm test` / `jest` / и т.д. Проверка корректности: `tsc --noEmit` в `backend/`, `npm run build` в `frontend/`, E2E — `curl` по ручкам после `make dev`.
